...
use shared::{AccountCredentials, AccountInfo, ClientControlStreamMessage, ServerControlStreamMessage};
use tokio::sync::mpsc::error::TryRecvError;

use crate::{engine::{Drawable, Graphics}, server_state::ServerState, ui::{Menu, MenuEvent, UIRoot, UIRootItem}};
    ...
    fn update(&mut self, _graphics: &Graphics) -> anyhow::Result<MenuEvent> {
        ...
        for result in self.server_state.clone().receive_messages()? {
            let message: ProcessedMessage = match result {
                ...
                Ok(msg) => {
                    eprintln!("LoadGameMenu cannot process message: {:?}", msg);
                    ProcessedMessage::None
                }
